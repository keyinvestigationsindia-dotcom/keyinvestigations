...
**Status: Phase 1 (this document's scope) implemented and deployed — see [phase-1-implementation-plan.md](phase-1-implementation-plan.md). This document itself remains the design record, kept current rather than frozen.** Feeds into (does not replace) the frozen [Legal & Investigation Intelligence Engine](../legal-intelligence/README.md).
...
**Platform `claim_types` grounding** (`supabase/migrations/20260509000003_multitenant.sql:127-153`) — the real, seeded claim types on this platform today are exactly: `motor_od`, `motor_theft`, `health`, `mact`, `tp`, `non_motor`. Two things worth stating plainly: (1) `report.html`'s own `claimType` dropdown — **updated since this section was first written**: now `["MACT Death Claim","MACT Injury Claim","TPPD Claim","Health Claim"]`, plus a matching entry in `linkClaim`'s `claim_type_id` → label map — is still a separate, hardcoded list, not sourced from this table, but the `health` gap it originally had is closed. (2) **PA/GPA and WC, named in the current architecture requirement, do not exist as `claim_types` rows anywhere in this codebase today** — they're named honestly below as future adapters, not implied to already have platform support.

**Storage availability — corrected this revision, an earlier finding here was wrong.** Every prior check in this document (`GET .../storage/v1/bucket` with the app's public anon key, repeatedly returning `HTTP 200` with body `[]`) was read as "zero buckets exist." That conclusion was incorrect: Supabase's Storage bucket-listing and bucket-lookup endpoints **do not expose private buckets to the `anon` role at all** — an empty/`NoSuchBucket` response from these endpoints means "anon can't see any," not "none exist." Checked directly against the project's own dashboard (authenticated, owner-level access — the only way to actually see this), the real state is: **three buckets already exist** — `documents` (4 access policies) and `reports` (0 policies), both pre-dating this proposal entirely and unrelated to it, plus **`intake-page-renders`**, created for this feature, private, with one authenticated-access policy (`bucket_id = 'intake-page-renders' AND auth.role() = 'authenticated'`). The bucket-creation prerequisite from earlier revisions of this document is therefore done, not merely "available" — see §11's updated row. The broader claim that this app previously had "no existing Storage usage" no longer holds either — `documents`/`reports` predate this whole proposal; this app's Report Drafter specifically had none, but the platform did.
...
| Storage bucket availability | **Resolved** — `intake-page-renders` bucket created, private, one authenticated-access policy configured, confirmed via dashboard (§2). The anon-key API check that earlier suggested zero buckets existed was itself wrong (private buckets aren't anon-visible) — corrected in §2, not just this row |
