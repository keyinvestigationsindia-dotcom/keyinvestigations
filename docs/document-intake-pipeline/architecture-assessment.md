...
**Revision history**: v1 (initial) → v2 (full-bundle batching architecture, mandatory review, 300-page configurable ceiling) → **v3, this version** (server-side encrypted storage for page images, a richer multi-signal boundary classifier plus a new global consistency pass, and server-side/auditable review state replacing client-only state).
...
- **TTL / cleanup**: **configurable**, not hardcoded — a retention window (e.g., default candidate: 30 days, matching roughly the outer bound of a realistic review cycle) after which a scheduled cleanup removes the objects. Needs to outlive the review session (§7) but should not become de facto permanent storage of case document images by accident — this is the one place in the whole proposal where an un-configurable default would be a real risk (silently keeping sensitive medical document images indefinitely).
...
| Page image storage becoming de facto permanent | **New** — mitigated by configurable TTL (§4), but the *default* TTL value is not yet chosen — placeholder in §4, needs a real decision before implementation |
| New infrastructure dependency (Storage bucket) not yet confirmed available/configured on this Supabase project's plan | **New** — needs a direct check against the actual project (`mqsohzqbsupsathxphgd`) before implementation, not assumed |
